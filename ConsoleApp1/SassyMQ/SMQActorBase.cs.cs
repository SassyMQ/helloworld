...
namespace SMQ.SassyMQ.Lib.RabbitMQ
{
    ...
    public abstract class SMQActorBase
    {
            ...
            this.MonitorTask = new Task(() =>
            {
                ...
                while (this.RMQChannel.IsOpen)
                {
                    ...
                    if (gotMessage)
                    {
                        ...
                        StandardPayload payload = JsonConvert.DeserializeObject<StandardPayload>(body);
        ...
        protected void Reply(StandardPayload payload, string replyTo)
        {
            IBasicProperties props = this.RMQChannel.CreateBasicProperties();
            props.CorrelationId = payload.PayloadId.ToString();
            this.CheckPayload(payload);
            var payloadJson = JsonConvert.SerializeObject(payload);
            this.RMQChannel.BasicPublish("", replyTo, props, Encoding.UTF8.GetBytes(payloadJson));
        }
    }
